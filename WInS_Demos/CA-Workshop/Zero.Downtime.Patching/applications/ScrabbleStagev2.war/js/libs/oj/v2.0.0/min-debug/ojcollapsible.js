...
define(["ojs/ojcore", "jquery", "ojs/ojcomponentcore"], function($oj$$29$$, $$$$28$$) {
  (function() {
    ...
    $oj$$29$$.$__registerWidget$("oj.ojCollapsible", $$$$28$$.oj.baseComponent, {widgetEventPrefix:"oj", options:{expanded:!1, disabled:null, expandOn:"click", expandArea:"header", beforeExpand:null, expand:null, beforeCollapse:null, collapse:null, optionChange:null}, _ComponentCreate:function() {
      ...
      var $elem$$66$$ = this.element[0];
      this.$_expandCollapseHandler$({type:this.options.expanded ? "ojexpand" : "ojcollapse", target:$elem$$66$$, currentTarget:$elem$$66$$, preventDefault:$$$$28$$.noop});
      this.$_initialRender$ = void 0;
    }, $_NotifyContextMenuGesture$:function($menu$$15$$, $event$$384$$, $eventType$$44$$) {
      this.$_OpenContextMenu$($event$$384$$, $eventType$$44$$, {launcher:this.element.find(".oj-collapsible-header-icon").first()});
    ...
    }, _setOption:function($key$$141$$, $value$$256$$, $flags$$37$$) {
      "expanded" === $key$$141$$ ? $value$$256$$ !== this.options.expanded && this.$_setExpanded$($value$$256$$) : "disabled" === $key$$141$$ ? (this.header.add(this.header.next()), this.element.toggleClass("oj-disabled", !!$value$$256$$)) : "expandOn" === $key$$141$$ || "expandArea" === $key$$141$$ ? (this.$_tearDownEvents$(), this._super($key$$141$$, $value$$256$$, $flags$$37$$), this.$_setupEvents$()) : this._super($key$$141$$, $value$$256$$, $flags$$37$$);
    }, $_keydown$:function($event$$385$$) {
      if (!$event$$385$$.altKey && !$event$$385$$.ctrlKey) {
        var $keyCode$$18$$ = $$$$28$$.ui.keyCode;
        switch($event$$385$$.keyCode) {
          case $keyCode$$18$$.SPACE:
          ;
          case $keyCode$$18$$.ENTER:
            this.$_toggleHandler$($event$$385$$);
        }
      }
    }, refresh:function() {
      this._super();
      ...
      var $events$$3$$ = {keydown:this.$_keydown$}, $event$$386_expandArea$$ = this.options.expandOn;
      if ($event$$386_expandArea$$) {
        var $self$$153$$ = this;
        $$$$28$$.each($event$$386_expandArea$$.split(" "), function($index$$195$$, $eventName$$1$$) {
          $oj$$29$$.$DomUtils$.$isValidIdentifier$($eventName$$1$$) && ($events$$3$$[$eventName$$1$$] = $self$$153$$.$_toggleHandler$);
        });
      }
      $event$$386_expandArea$$ = this.element.find(this.$_getExpandAreaSelector$());
      this._on($event$$386_expandArea$$, $events$$3$$);
      ...
      this.$_isDisabled$() || (this._on(this.element, {ojexpand:this.$_expandCollapseHandler$, ojcollapse:this.$_expandCollapseHandler$, ojfocus:this.$_focusHandler$, ojfocusout:this.$_focusHandler$}), this._hoverable($event$$386_expandArea$$), this._focusable($event$$386_expandArea$$), this.$_activeable$($event$$386_expandArea$$));
    ...
    }, $_toggleHandler$:function($event$$387$$) {
      this.$_isDisabled$() || $event$$387$$.isDefaultPrevented() || (this.options.expanded ? this.collapse(!0, $event$$387$$) : this.expand(!0, $event$$387$$), $event$$387$$.preventDefault(), $event$$387$$.stopPropagation(), this.header.find(".oj-collapsible-header-icon").focus());
    }, $_expandCollapseHandler$:function($event$$388$$) {
      if (!this.$_isDisabled$() && $event$$388$$.target === this.element[0] && (this.$_initialRender$ || !$event$$388$$.isDefaultPrevented())) {
        var $element$$116$$ = this.element, $options$$298$$ = this.options, $content$$22$$ = this.content, $wrapper$$ = this.$wrapper$, $isExpanded$$ = "ojexpand" === $event$$388$$.type;
        $event$$388$$.preventDefault();
        this.$_initialRender$ ? ($options$$298$$.expanded = $isExpanded$$) ? ($element$$116$$.removeClass("oj-collapsed"), $element$$116$$.addClass("oj-expanded"), $oj$$29$$.Components.$subtreeShown$($wrapper$$[0])) : ($element$$116$$.removeClass("oj-expanded"), $element$$116$$.addClass("oj-collapsed"), $wrapper$$.css("max-height", 0), $wrapper$$.hide(), $oj$$29$$.Components.$subtreeHidden$($wrapper$$[0])) : (this.$_changeExpandedOption$($isExpanded$$, $event$$388$$), $wrapper$$.$contentHeight$ = 
        $wrapper$$.outerHeight(), $isExpanded$$ ? ($wrapper$$.show(), setTimeout(function() {
          ...
          $element$$116$$.removeClass("oj-collapsed");
          $element$$116$$.addClass("oj-expanded");
          ...
          $element$$116$$.removeClass("oj-expanded");
          $element$$116$$.addClass("oj-collapsed");
    ...
    }, $_focusHandler$:function($event$$389$$) {
      if (this.$_isDisabled$()) {
        return null;
      }
      "ojfocusout" == $event$$389$$.type ? (this.$_findFirstFocusableInHeader$().attr("tabIndex", -1), $event$$389$$.preventDefault(), $event$$389$$.stopPropagation()) : "ojfocus" == $event$$389$$.type && (this.$_findFirstFocusableInHeader$().attr("tabIndex", 0).focus(), $event$$389$$.preventDefault(), $event$$389$$.stopPropagation());
    ...
    }, expand:function($vetoable$$4$$, $event$$390$$) {
      if (!this.$_isDisabled$()) {
        var $eventData$$14$$ = {header:this.header, content:this.content};
        $vetoable$$4$$ && !1 === this._trigger("beforeExpand", $event$$390$$, $eventData$$14$$) || this._trigger("expand", $event$$390$$, $eventData$$14$$);
      }
    }, collapse:function($vetoable$$5$$, $event$$391$$) {
      if (!this.$_isDisabled$()) {
        var $eventData$$15$$ = {header:this.header, content:this.content};
        $vetoable$$5$$ && !1 === this._trigger("beforeCollapse", $event$$391$$, $eventData$$15$$) || this._trigger("collapse", $event$$391$$, $eventData$$15$$);
      }
    }, $_transitionEndHandler$:function($event$$392$$) {
      this.$_isDisabled$() || ("max-height" == ($event$$392$$.originalEvent ? $event$$392$$.originalEvent.propertyName : null) && ($event$$392$$.preventDefault(), $event$$392$$.stopPropagation()), this.options.expanded ? this.$wrapper$.css({"max-height":9999, "overflow-y":""}) : this.$wrapper$.hide(), this.$wrapper$.removeClass("oj-collapsible-transition"));
    }, $_changeExpandedOption$:function($value$$257$$, $originalEvent$$1$$) {
      this.option("expanded", $value$$257$$, {_context:{originalEvent:$originalEvent$$1$$, $writeback$:!0, $internalSet$:!0}});
