...
![alt text](https://github.com/oracle-weblogic/weblogic-innovation-seminars/tree/caf-12.2.1/WInS_Demos/MT-Workshop/images/1.png  "Multitenancy Architecture")
...
12b.	Check the box near to **app-cluster** to make it highlighted and then Click on **Control -> Start -> Start Servers**.
...
Virtual Target: 		VT-Medrec-1
Domain Partition:	dp1
Resource Group 	app1RG
app1RG:     
	Datasource:	 	
	...
	Persistence Store:	MedRec-fs
	JMS Server:		MedRecJMSServer
	JMS Module:		MedRecModule
	MedRecModule:	
		Subdeployment:	MedRecJMS
		Connection Factory:	
			Name:		MedRecConnectionFactory 								JNDI Name:	com.oracle.medrec.jms.connectionFactory
		Distributed Queue:	
			Name:		PatientNotificationQueue
			JNDI Name:	com.oracle.medrec.jms.PatientNotificationQueue
	Applications:		medrec.ear
				physician.ear
				chat.war	
1.	Open a new tab.
...
25d.	Select **Oracle** as Database Type and ** “Oracle’s Driver (Thin XA) for service connections; Versions: Any” ** as JDBC Driver then click on **OK**.
25e.	 Click on **Next**.
25f.	Click on **Generate URL and Properties** and Enter the following:
		...
		Click **OK**.
25g.	Click on **Test Database Connection** to verify the connection. Click **Next**.
25h.	Leave Default on **Transaction Options** and click on **Next**.
25i.	Verify the configuration and click on **Create**.
26.	Adding User to Default Realm.
26a.	Click on **Weblogic Domain -> Security -> Users and Groups**.
26b.	In **Users** tab, click on **Create**.
26c.	Enter the following then click on **Create**.
		Name:			administrator
...
27a.	Click on **Weblogic Domain ->Environment ->Domain Partition**.
27b.	Click on Domain Partition **dp1**.
27c.	Click on **Domain Partition -> Other Services -> Mail Sessions**.
27d.	Click on **Create**. 
27e.	Enter the following and click on **Next**.
		Name:			MedRecMailSession
		Scope:			Leave as default
		JNDI Name:		mail/MedRecMailSession
27f.	Click on **Create**.
28.	Configuring JMS Server.
28a.	Click on **Domain Partition -> Messaging -> JMS Servers**.
28b.	Click on **Create**. Enter **MedRecJMSServer** as Name then click on **Create a Store   -> File Store**. 
28c.	Enter **MedRec-fs** as Name then click on **Next**. Click on **Create**.
28d.	In **JMS Server: General Setting**, Select the newly created file store as persistent store then click on **Next**.
28e.	Click on **Create**. 

29.	Configure JMS Module.
29a.	Click on **Domain Partition -> Messaging -> JMS Resources and Modules**. 
29b.	Under the **JMS Modules** tab, click on **Create**.
29c.	Enter **MedRecModule** as Name then click on **Next**. Click on **Create**. 
29d.	Under **JMS Modules** tab, click on **MedRecModule**.
29e.	 Under **Subdeployment** tab, click on **Create**. 
29f.	Enter **MedRecJMS** as Name and Select the box near **MedRecJMSServer** then click on **Create**. 
29g.	Under **General** tab, click on **Create**. 
29h.	Select box near **Connection Factory** then click on **Next**. 
29i.	Leave Default in **JMS System Module** Page then click on **Next**. 
29j.	Enter **MedRecConnectionFactory** as Name and **com.oracle.medrec.jms.connectionFactory** as JNDI Name then click on **Next**. Click on **Create**.
29k.	Under **General** tab, Click on **Create**. 
29l.	Select **Uniform Distributed Queue** then click on **Next**. 
29m.	Click on **Next**. 
29n.	Enter the following and click on **Next**. 
		Name: 		PatientNotificationQueue
		JNDI Name: 	com.oracle.medrec.jms.PatientNotificationQueue
29o.	Click on **Create**. Click on **PatientNotificationQueue**, Under **Targeting**, Select **Subdeployment targeting** as **Targeting Policy** and **MedRecJMS** as **Subdeployment** then click on **Apply**.
Note: You must restart domain partition before going to deploy Medrec Application.
1.	Click on Change Center -> View Restart Checklist.
2.	Select the box near to dp1 then click on Restart.
3.	On Confirmation Screen, Click on Restart.
...
30d.	Select ** “Archive or exploded directory is on the server where Enterprise Manager is running”** then click on **Browse**. Specify the location of **medrec.ear** from **/u01/content/weblogic-innovation-seminars/WInS_Demos/MT-Workshop/Lab2** then click on **OK**.
...
3.	Under Patient, Click on **I’m New Here**
...
app1RG:     
	Datasource:	 	MedRec2GlobalDataSourceXA, 										jdbc/MedRecGlobalDataSourceXA
	Mail Session:		MedRecMailSession, mail/MedRecMailSession
	Persistence Store:	MedRec2-fs
	JMS Server:		MedRec2JMSServer
	JMS Module:		MedRec2Module
	MedRecModule:	
		Subdeployment:	MedRec2JMS
		Connection Factory:	MedRec2ConnectionFactory, 										com.oracle.medrec.jms.connectionFactory
		Distributed Queue:	PatientNotificationQueue
...
6a.	Click on **WebLogic Domain ->Environment ->Virtual Target**. Here we have **dp2** as **URI Prefix** for the Virtual Target **VT-Medrec-2**.
...
6e.	In the ** “Services” ** and ** “Deployments” ** tab, you can verify the creation of above System Resources here.
...
Virtual Target: 		VT-daytrader
Domain Partition:	dp3
Resource Group 	app3RG
app1RG:     
	Datasource:	
		Name:	 		jdbc/datasources/TradeDataSource, 						JNDI Name:		jdbc/datasources/TradeDataSource
		Name:			jdbc/datasources/NoTxTradeDataSource
		JNDI Name:		jdbc/datasources/NoTxTradeDataSource
	Persistence Store:	MyFileStore
	JMS Server:		MyJMSServer
	JMS Module:		MyJMSModule
	MyJMSModule:	
		Subdeployment:	MySubdeployment
		Connection Factory:
			Name:			jms/myQueueConnectionFactory, 						JNDI Name:		jms/myQueueConnectionFactory
			Name:			jms/myTopicConnectionFactory
			JNDI Name:		jms/myTopicConnectionFactory
		Distributed Queue:
...
5a.	Click on **WebLogic Domain ->Environment ->Virtual Target**. Here we have **dp3** as **URI Prefix** for the Virtual Target **VT-daytrader**.
5b.	Click on **WebLogic Domain ->Environment ->Domain Partition**. 
5c.	Click on Domain Partition to **dp3** then Select **Domain Partition -> Administration -> Resource Groups**.
5d.	Click on Resource group **app3RG**.
5e.	In **Services** and **Deployments** tab, you can verify the creation of above System Resources here.
...
6a.	Click on **Environment -> Virtual Targets**.
6b.	Click on New.
6c.	Enter the following then click on **OK**.
	Name:		VT-Medrec-1 (Both Virtual Target Name will be same)
	Target:		Admin Server
	URI Prefix:	/devDP	
7.	Creation of Domain Partition.
7a.	Click on **Domain Partition -> New**.
7b.	Enter **Medrec-Dev** as Name and **uncheck** the box for **Default Resource Group**. Click on **Next**.
7c.	Check the box for **VT-Medrec-1** then click on **Next**. 
7d.	In Default Targets, Move **VT-Medrec-1** to Chosen. Click on **Finish**.
7e.	In **Domain Partition -> Control tab**, check the box near Medrec-Dev and click on **Start**. 
8.	Creation of Resource Group
8a.	Click on domain partition **Medrec-Dev**. 
8b.	Click on **Resource Groups -> Configuration tab** then click on New.
8c.	Enter **appRG** as Name, Leave others as default then click on **OK**.
...
9a.	Click on **dev_domain -> Services -> Data Sources** then click on **New-> Generic Data Sources**.
9b.	Enter the following and click on **Next**.
Name:		MedRecGlobalDataSourceXA
JNDI Name:		jdbc/MedRecGlobalDataSourceXA
Scope:		appRG in Medrec-Dev
Database Type:	Oracle
9c.	Select **“Oracle’s Driver (Thin XA) for Service connections; Version: Any “** then click on Next.
...
9e.	Enter the following and click on **Next**. 
	...
	Password:			medrec1
9f.	Click on **Test Configuration** and verify the connectivity.
9g.	Click on **Next** then Click on **Finish**.
10.	Creation of JMS Server.
10a.	Click on **Services -> Messaging -> JMS Servers**.
10b.	Click on New. 
10c.	Enter the following and click on **Next**.
	Name:		DevJMSServer
	Scope:		appRG in Medrec-Dev
10d.	Click on **Create a New Store**.
10e.	Select **“File Store”** as Type then click on **Next**.
10f.	Enter **“MedrecDev-fs”** as Name and **“appRG in Medrec-Dev”** as Scope then click on **Next**. Click on **Finish**. 
10g.	Select **“MedrecDev-fs”** as Persistent Store then click on **Next**.
10h.	Click on **Finish**.
11.	Creation of JMS Module.
11a.	Click on **Services -> Messaging -> JMS Modules** then click on New.
11b.	Enter **DevJMSModule** as Name and **“appRG in Medrec-Dev”** then click on **Next**. 
11c.	Check the box for **“Would you like to add resources to this JMS system module”** then click on **Finish**. 
...
12c.	Select **DevJMSServer** as Target then click on **Finish**.
13.	Creation of Connection Factory.
13a.	Click on Configuration tab then click on New. 
13b.	Select the box for Connection Factory then click on Next.
13c.	Enter **DevMedRecConnectionFactory** as Name and **com.oracle.medrec.jms.connectionFactory** and leave other default then click on **Next**.
13d.	Click on **Finish**.
14.	Creation of Distributed Queue.
14a.	Click on **New**. 
14b.	Select the box for Distributed Queue then click on **Next**.
14c.	Enter **PatientNotificationQueue** as Name and **com.oracle.medrec.jms.PatientNotificationQueue** as JNDI Name and leave other default then click on **Next**. 
14d.	Click on **Advanced Targeting**, Select DevMedrecJMS as Subdeployments and DevJMSServer as Targets then click on **Finish**. 
15.	Creation of Mail Session. 
15a.	Click on **Services -> Mail Sessions**. 
15b.	Click on **New**.
15c.	Enter the following then click on **Next**.
	Name:			MedRecMailSession
	JNDI Name:		mail/MedRecMailSession
	Scope:			appRG in Medrec-Dev
15d.	Click on **Finish**.
...
16c.	Select **“Install this deployment as an application”** and **“appRG in Medrec-Dev”** as Scope then click on **Next**. Click on **Finish**. 
...
16e.	 Select **“Install this deployment as an application”** and **“appRG in Medrec-Dev”** as Scope then click on **Next**. Click on **Finish**.
...
16g.	Select **“Install this deployment as an application”** and **“appRG in Medrec-Dev”** as Scope then click on **Next**. Click on **Finish**.
...
11.	In this tab, Click on Enter **Terminal -> Set Title** and **app-cluster-1** then click on **OK**.
	We will use these logs to monitor resource consumption manager lab.
...
24b.	Click on Create, Enter **“otd_machine”** as Name and **“Unix”** as Machine OS then Click on **Next**.
...
25b.	Click on **Create**.
25c.	Enter **“mt”** as Name and **“HTTP”** as **Origin Server Type** then click on **Next**.
25d.	Leave Default in **“Create Configuration: Listener”** then click on **Next**.
25e.	Leave Default in **“Create Configuration: Server Pool”** then click on **Next**.
25f.	Select the **“otd_machine”** then click on **Next**.
25g.	Click on **“Create Configuration”**.
25h.	Check the box near **“mt”** to make it highlighted and then click on **Start Instances**. Click on **Close**.
...
2a.	Click on **WebLogic Domain -> Environment -> Clusters**.
...
2e.	Select the box for **“Use a single machine for all dynamic servers”** and choose **“machine”** as Selected Machine then click on **Next**.
...
2i.	Check the box near **new-cluster** to make it highlighted and then click on **Control -> Start -> Start Servers**.
...
3c.	Once you notice **“Migrating resource group “app4RG” – Completed successfully”** message then click on Close.
