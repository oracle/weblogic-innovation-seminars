...
Accessing Medrec application in dp1: [alt text](http://{PUBLIC IP OF JCS INSTANCE}/dp1/medrec)
Accessing Medrec application in dp2: [alt text] (http://{PUBLIC IP OF JCS INSTANCE}/dp2/medrec)  
Accessing Daytrader application in dp3: [alt text] (http://{PUBLIC IP OF JCS INSTANCE}/dp3/daytrader) 
...
2. **./Lab4_cloud.sh**
This Lab4_cloud.sh scripts start the Database in local Virtual box, then it creates the user and populate the database with sample data. Then it creates a dev_domain, and starts admin server in it, later we configure all the resources required for conference planner application. This is simple application which we used during Java One in 2011 for registration purpose for various events. At the end of execution of this script, you can access the application on [alt text](http://localhost:9001/dp6/ConferencePlanner/) .

3. Open a new tab in browser, and type the URL for Admin Console for dev_domain [alt text] (http://localhost:9001/console) .
...
14. Go to Admin Console of JCS instance http://localhost:7001/console and enter weblogic/welcome1 as Username/Password and then click on Login.
...
20. Click on the refresh icon, once the partition is in RUNNING state, go to the browser and access the application http://{PUBLIC_IP_OF_JCS_INSTANCE/dp6/ConferencePlanner/ .
...
4. chmod -Rf 777 /tmp
5. Go back to Admin Console of winsdemoWLS_domain http://localhost:7001/console .
