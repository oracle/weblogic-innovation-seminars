...
domain_Name = 'jms-cluster-domain'
...
def createJMSModule(moduleName,
                    clusterMBean,
                    clusteredJMSServerMBean,
                    connectionFactoryNames,
                    queueNames,
                    partitionedTopicNames):
  ...
  print 'Creating subdeployment: cluster-subdeployment for cluster: ' + str(clusterMBean)

  cd('/JMSSystemResources/' + moduleName)

  subdeployment = create('cluster-subdeployment', 'SubDeployment')
  subdeployment.setTargets(jarray.array([clusteredJMSServerMBean], weblogic.management.configuration.TargetMBean))

  print 'Creating Connection Factories...'

  for cfName in connectionFactoryNames:
    print 'Creating JMS ConnectionFactory: ' + cfName
    cd('/JMSSystemResource/' + moduleName + '/JmsResource/NO_NAME_0')
    myCF = create(cfName, 'ConnectionFactory')
    myCF.setJNDIName(cfName)
    myCF.setDefaultTargetingEnabled(true)

    cd('/JMSSystemResources/' + moduleName + '/JmsResource/NO_NAME_0/ConnectionFactories/' + cfName)
    lbParams = create(cfName, 'LoadBalancingParams')
    lbParams.setLoadBalancingEnabled(true)
    lbParams.setServerAffinityEnabled(false)

    txParams = create(cfName, 'TransactionParams')
  ...
  for qName in queueNames:
    ...
    queue = create(qName, 'UniformDistributedQueue')
    queue.setJNDIName(qName)
    queue.setDefaultTargetingEnabled(false)
    queue.setSubDeploymentName('cluster-subdeployment')

  #  for rdtName in replicatedTopicNames:
  #    print 'Creating JMS Replicated Topic: ' + rdtName
  #    cd('/JMSSystemResources/' + moduleName + '/JmsResource/NO_NAME_0')
  #    topic = create(rdtName, 'UniformDistributedTopic')
  #    topic.setJNDIName(rdtName)
  ...
  for pdtName in partitionedTopicNames:
    print 'Creating JMS Partitioned Topic: ' + pdtName
    cd('/JMSSystemResources/' + moduleName + '/JmsResource/NO_NAME_0')
    topic = create(pdtName, 'UniformDistributedTopic')
    topic.setJNDIName(pdtName)
    topic.setForwardingPolicy('Partitioned')
    topic.setDefaultTargetingEnabled(false)
    topic.setSubDeploymentName('cluster-subdeployment')


def createBaseJMSResources(moduleName, clusterMBean, clusteredJMSServerMBean):
  print '### createBaseJMSResources ###################################################################################'

  cfNames = ['com.oracle.example.jms.base.cf']

  queueNames = ['com.oracle.example.jms.base.queue']

  pdtNames = ['com.oracle.example.jms.base.partitioned-topic',
              'com.oracle.example.jms.base.clearscreen',
              'com.oracle.example.jms.base.topic']

  createJMSModule(moduleName,
                  clusterMBean,
                  clusteredJMSServerMBean,
                  cfNames,
                  queueNames,
...
def createOPSJMSResources(moduleName, clusterMBean, clusteredJMSServerMBean):
  print '### createOPSJMSResources ####################################################################################'

  cfNames = ['com.oracle.demo.ops.jms.cf']

  queueNames = ['com.oracle.demo.ops.jms.eventQueue',
                'com.oracle.demo.ops.jms.shipmentQueue']

  pdtNames = ['com.oracle.demo.ops.jms.eventTopic']
...
def createMigrationJMSResources(moduleName, clusterMBean, clusteredJMSServerMBean):
...
def createWLDFJMSResources(moduleName, clusterMBean, clusteredJMSServerMBean):
  print '### createWLDFJMSResources ###################################################################################'

  cfNames = ['com.oracle.example.jms.util.cf',
             'com.oracle.example.jms.wldf.cf']

  queueNames = ['com.oracle.example.jms.wldf.notification',
                'com.oracle.example.jms.util.deadlock',
                'com.oracle.example.jms.util.jdbcHogger'
                'com.oracle.example.jms.util.stuckThreadGenerator']
  pdtNames = []
...
def createUOOResources(moduleName, clusterMBean, clusteredJMSServerMBean):
  print '### createUOOResources #######################################################################################'

  cfNames = ['com.oracle.example.jms.uoo.cf']
  queueNames = ['com.oracle.example.jms.uoo.queue']
  pdtNames = []
...
def createUOWResources(moduleName, clusterMBean, clusteredJMSServerMBean):
  print '### createUOWResources #######################################################################################'

  cd('/')
  myJmsSystemResource = create(moduleName, 'JMSSystemResource')
  myJmsSystemResource.setTargets(jarray.array([clusterMBean], weblogic.management.configuration.TargetMBean))
...
def createSpringJMSTempResources(moduleName, clusterMBean, clusteredJMSServerMBean):
  print '### createSpringJMSTempResources #############################################################################'

  cfNames = ['com.oracle.example.jms.spring.cf']
  queueNames = ['com.oracle.example.jms.spring.queue']
  pdtNames = []
...
def createSAFStoresAndAgents(clusterMBean, clusteredJMSServerMBean):
  print '### createSAFStoresAndAgents #################################################################################'

  cd('/')
  jdbcStoreName = 'saf-store-cluster'
  jdbcStore = create(jdbcStoreName, 'JDBCStore')
  jdbcStore.setDataSource(jdbcSystemResource)
  jdbcStore.setPrefixName('SAFSTORECLUSTER')

  targets = jarray.array([clusterMBean], weblogic.management.configuration.TargetMBean)
  jdbcStore.setTargets(targets)

  cd('/')
  safAgent = create('saf-agent-cluster', 'SAFAgent')
...
def createSAFSourceModules(clusterMBean, clusteredJMSServerMBean):
  print '### createSAFSourceModules ###################################################################################'

  cfNames = ['com.oracle.example.jms.saf.cf']
  qNames = ['com.oracle.example.jms.saf.local-queue']
  pdtNames = []

  createJMSModule('jms-module-saf-source',
                  clusterMBean,
                  clusteredJMSServerMBean,
                  cfNames,
                  qNames,
  ...
  ########################################################################################################################
  print 'Creating SAFRemoteContext'
  cd('/JMSSystemResources/jms-module-saf-source/JmsResource/NO_NAME_0')
  safRemoteContext = create('remote-saf-context-1', 'SAFRemoteContext')

  cd('/JMSSystemResources/jms-module-saf-source/JmsResource/NO_NAME_0/SAFRemoteContexts/remote-saf-context-1/')
  ...
  print 'Creating SAFErrorHandling'
  cd('/JMSSystemResources/jms-module-saf-source/JmsResource/NO_NAME_0')
  ...
  print 'Creating SAFQueue'
  cd(
    '/JMSSystemResources/jms-module-saf-source/JmsResource/NO_NAME_0/SAFImportedDestinations/saf-imported-destinations')
...
def createJMSModules(clusterMBean, clusteredJMSServerMBean):
  print '### Creating JMS Modules #####################################################################################'
  createBaseJMSResources('jms-module-base', clusterMBean, clusteredJMSServerMBean)
  createUOOResources('jms-module-uoo', clusterMBean, clusteredJMSServerMBean)
  createWLDFJMSResources('jms-module-wldf', clusterMBean, clusteredJMSServerMBean)
  createOPSJMSResources('jms-module-ops', clusterMBean, clusteredJMSServerMBean)
  createSpringJMSTempResources('jms-module-temp', clusterMBean, clusteredJMSServerMBean)
  createSAFSourceModules(clusterMBean, clusteredJMSServerMBean)
  createSAFStoresAndAgents(clusterMBean, clusteredJMSServerMBean)
  createMigrationJMSResources('jms-module-migration', clusterMBean, clusteredJMSServerMBean)
  createUOWResources('jms-module-uow', clusterMBean, clusteredJMSServerMBean)
...
try:
  ...
  jmsServerName = 'Cluster-JMS-Server'
  jdbcStoreName = jmsServerName + '-jdbcStore'

  jmsServerTargets = jarray.array([clusterMBean], weblogic.management.configuration.TargetMBean)

  print 'Creating JMS JDBC Store [' + jdbcStoreName + ']'
  cd('/')
  jdbcStore = create(jdbcStoreName, 'JDBCStore')
  jdbcStore.setDataSource(jdbcSystemResource)
  jdbcStore.setPrefixName('JMSSTORE' + str(0))
  ...
  createJMSModules(clusterMBean, jmsServer)
