...
public class Snake {
    ...
    private final WebSocketConnection outbound;
    ...
    public Snake(int id, WebSocketConnection outbound) {
        this.id = id;
        this.outbound = outbound;
        this.hexColor = SnakeWebSocketWLS.getRandomHexColor();
        resetState();
    }

    private void resetState() {
        this.direction = Direction.NONE;
        this.head = SnakeWebSocketWLS.getRandomLocation();
        this.tail.clear();
    ...
    private synchronized void kill() {
        ...
        try {
            ...
            outbound.send(response.toString());
    ...
    public synchronized void update(Collection<Snake> snakes) {
        ...
        if (nextLocation.x >= SnakeWebSocketWLS.PLAYFIELD_WIDTH) {
            nextLocation.x = 0;
        }
        if (nextLocation.y >= SnakeWebSocketWLS.PLAYFIELD_HEIGHT) {
            nextLocation.y = 0;
        }
        if (nextLocation.x < 0) {
            nextLocation.x = SnakeWebSocketWLS.PLAYFIELD_WIDTH;
        }
        if (nextLocation.y < 0) {
            nextLocation.y = SnakeWebSocketWLS.PLAYFIELD_HEIGHT;
        }
