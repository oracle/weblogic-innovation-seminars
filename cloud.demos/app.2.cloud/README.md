...
- Provided VirtualBox image or [custom environment prepared](https://github.com/oracle-weblogic/weblogic-innovation-seminars/blob/caf-12.2.1/cloud.demos/jcs.basics/vbox.vm.md) for this tutorial.
- Running and "empty" [Database Cloud Service](https://github.com/oracle-weblogic/weblogic-innovation-seminars/blob/caf-12.2.1/cloud.demos/jcs.basics/create.dbcs.ui.md) instance which has no Java Cloud Services dependency.

### Steps ###

#### Create Weblogic 10.3.6 domain and deploy the Petstore sample application ####
Open a terminal and change to `/u01/content/weblogic-innovation-seminars/cloud.demos/app.2.cloud` folder.

    $ [oracle@localhost Desktop]$ cd /u01/content/weblogic-innovation-seminars/cloud.demos/app.2.cloud
Run the `prepareEnv.sh` script which starts the database, creates Weblogic 10.3.6 domain, starts Weblogic servers and deploys the Petstore demo application. The script usage is: `prepareDBCS.sh <db user> <db password> <ssh key file> <db server ip> [<PDB name>]`. In the provided virtualbox environment run the script with the following parameters:
	...
	********** CREATING PETSTORE_DOMAIN (WEBLOGIC 10.3.6 - PETSTORE_DOMAIN) ***********
	<< read template from "/u01/content/weblogic-innovation-seminars/cloud.demos/app.2.cloud/template/petstore_domain_template.jar"
	>>  succeed: read template from "/u01/content/weblogic-innovation-seminars/cloud.demos/app.2.cloud/template/petstore_domain_template.jar"
